...
(function() {
    ...
    class CommentSystem {
        constructor(articleId, config) {
            ...
            this.container = null;
            this.config = config;
            this.articleAuthorId = null; // Set this if you know the article author's UID
            
            console.log('CommentSystem initializing for article:', articleId);
            this.initFirebase();
            this.initAuth();
        }
        ...
        render() {
            if (!this.container) {
                console.error('No container to render to');
                return;
            }
            ...
            console.log('Rendering comment section, authenticated:', isAuthenticated);
        ...
        attachEventListeners() {
            ...
            document.querySelectorAll('.sort-btn').forEach(btn => {
                btn.addEventListener('click', (e) => {
                    ...
                    if (!target) return;
                    this.sortBy = target.dataset.sort;
                    // Update active class
                ...
                <div class="comment-item ${isPinned ? 'pinned' : ''} ${comment.isCreatorHearted ? 'creator-hearted' : ''}" data-comment-id="${comment.id}">
                    <div class="comment-header">
                        <div class="comment-user">
                            <img src="${comment.userAvatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(comment.userName || 'User')}&background=1a73e8&color=fff`}" 
                                 alt="${comment.userName || 'User'}" 
                                 class="comment-user-avatar"
                                 onerror="this.src='https://ui-avatars.com/api/?name=${encodeURIComponent(comment.userName || 'User')}&background=1a73e8&color=fff'">
                    ...
                    <div class="comment-content">
                        ${this.editingCommentId === comment.id ? `
                            ...
                            <div class="comment-edit-actions">
                                <button class="cancel-btn" onclick="window.commentSystem.cancelEdit()">Cancel</button>
                                <button class="submit-btn" onclick="window.commentSystem.saveEdit('${comment.id}')">Save</button>
                            </div>
                        ` : `
                    ...
                    <div class="comment-footer">
                        <div class="comment-actions">
                            <button class="comment-action-btn ${isLiked ? 'liked' : ''}" onclick="window.commentSystem.toggleLike('${comment.id}')">
                                <span class="material-icons-round">${isLiked ? 'favorite' : 'favorite_border'}</span>
                                ${comment.likes || 0}
                            </button>
                            
                            <button class="comment-action-btn" onclick="window.commentSystem.startReply('${comment.id}')">
                                <span class="material-icons-round">reply</span>
                                Reply
                            </button>
                            
                            ${canEdit ? `
                                <button class="comment-action-btn" onclick="window.commentSystem.startEdit('${comment.id}')">
                            ...
                            ${canDelete ? `
                                <button class="comment-action-btn" onclick="window.commentSystem.openDeleteModal('${comment.id}')">
                            ...
                            ${canPin ? `
                                <button class="comment-action-btn" onclick="window.commentSystem.togglePin('${comment.id}')">
                            ...
                            ${canHeart ? `
                                <button class="comment-action-btn" onclick="window.commentSystem.toggleCreatorHeart('${comment.id}')">
        ...
        renderReplies(commentId, replies) {
            ...
            return `
                <div class="replies-section">
                    <button class="replies-toggle" onclick="window.commentSystem.toggleReplies('${commentId}')">
                ...
                <div class="reply-item" data-reply-id="${reply.id}">
                    <div class="reply-header">
                        <div class="reply-user">
                            <img src="${reply.userAvatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(reply.userName || 'User')}&background=1a73e8&color=fff`}" 
                                 alt="${reply.userName || 'User'}" 
                                 class="reply-user-avatar"
                                 onerror="this.src='https://ui-avatars.com/api/?name=${encodeURIComponent(reply.userName || 'User')}&background=1a73e8&color=fff'">
                            <span class="reply-user-name">${reply.userName || 'User'}</span>
                        </div>
                    ...
                    <div class="reply-actions">
                        <button class="reply-action-btn ${isLiked ? 'liked' : ''}" onclick="window.commentSystem.toggleReplyLike('${commentId}', '${reply.id}')">
                            <span class="material-icons-round">${isLiked ? 'favorite' : 'favorite_border'}</span>
                            ${reply.likes || 0}
                        </button>
                        
                        ${canDelete ? `
                            <button class="reply-action-btn" onclick="window.commentSystem.deleteReply('${commentId}', '${reply.id}')">
                                <span class="material-icons-round">delete</span>
                                Delete
                            </button>
                        ` : ''}
            ...
            return `
                <div class="reply-form">
                    ...
                    <div class="reply-form-actions">
                        <button class="cancel-btn" onclick="window.commentSystem.cancelReply()">Cancel</button>
                        <button class="submit-btn" onclick="window.commentSystem.submitReply('${commentId}')">
                            <span class="material-icons-round">send</span>
                            Post Reply
                        </button>
                    </div>
        ...
        attachCommentEventListeners() {
            // No additional listeners needed as we're using onclick handlers
        }
        ...
        async submitComment() {
            ...
            const commentData = {
                ...
                userName: this.currentUser.displayName || this.currentUser.email || 'User',
            ...
            try {
                ...
                input.value = '';
                const countSpan = document.getElementById('commentCharCount');
                if (countSpan) countSpan.textContent = '0/2000';
        ...
        signIn() {
            ...
            if (window.ReverbitAuth && window.ReverbitAuth.signIn) {
    ...
    function initCommentSystem() {
        ...
        if (!container) {
            console.error('Comments container not found! Make sure <div id="comments-container"></div> exists in your HTML.');
            return;
        }
